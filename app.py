...
# DATABASE_SERVERNAME = os.getenv("DATABASE_SERVERNAME")
...
#
# # Initialize session state for error tracking if not already set
# if 'error_message' not in st.session_state:
#     st.session_state['error_message'] = None
#
#
# # Function to fetch data from MSSQL based on input query
...
#     cursor.close()
#
#     # Check if rows and columns align
...
#
#
# # Function to query total daily LDA report frequency by issue area
# def fetch_daily_lda_frequency_by_issue_area(start_year, end_year):
#     query = f"""
#         SELECT DISTINCT si.issue_name, si.issue_description, lr.report_year AS Year, lrt.report_quarter AS Quarter,
#         lr.signed_date AS Date
#         FROM legis.dbo.lobby_report AS lr
#         LEFT JOIN legis.dbo.lobby_report_type AS lrt
#         ON lr.lobby_report_type_id = lrt.lobby_report_type_id
#         LEFT JOIN legis.dbo.lobby_report_issue AS lri
#         ON lr.lobby_report_id = lri.lobby_report_id
#         LEFT JOIN legis.dbo.lobby_issue AS li
#         ON lri.lobby_issue_id = li.lobby_issue_id
#         LEFT JOIN legis.dbo.swi_issue_lobbying_crosswalk AS sic
#         ON li.lobby_issue_id = sic.lobby_issue_id
#         LEFT JOIN legis.dbo.swi_issue AS si
#         ON sic.swi_issue_id = si.swi_issue_id
#         WHERE lr.amended_lobby_report_id IS NULL AND lr.lobby_report_type_id IN (10, 11, 14, 15, 18, 19, 22, 23)
#         AND lr.report_year >= {start_year} AND lr.report_year <= {end_year}
#         AND lr.is_deleted = 0 AND lri.is_deleted = 0 AND li.is_deleted = 0 AND sic.is_deleted = 0 AND si.is_deleted = 0
#         GROUP BY si.issue_name, si.issue_description, lr.report_year, lrt.report_quarter, lr.signed_date
#     """
#     return fetch_data(query)

# lda_frequency_by_issue_area_over_time = fetch_daily_lda_frequency_by_issue_area(start_year=1900, end_year=2024)
...
# Function to fetch the CSV file from S3 and load it as a DataFrame
@st.cache_data
def fetch_csv_from_s3(bucket_name, folder_name, file_name):
    s3 = boto3.client('s3')
...
# Streamlit App
st.title("Interactive Lobbying Data by Issue Area Dashboard")
